...
public class CDTHelperUtils {
	...
	public static final CDTHelper fromQualifiedName(TypeService ts, String namespace, String typeName) {
	...
	public static final CDTHelper fromTypeId(TypeService ts, Long typeId) throws InvalidTypeException {
		return fromDatatype(ts.getTypeSafe(typeId));
	}
	...
	public static final CDTHelper fromDatatype(Datatype dtype) {
		return fromNamedTypedValue(dtype.getTypeof(), dtype.getInstanceProperties());
	}

	public static final CDTHelper fromTypedValue(TypeService ts, TypedValue tv) throws InvalidTypeException {
		Long id=tv.getInstanceType();
		return fromTypeId(ts,id);
	}
	...
	public static Object[][] getObjectMultiCDT(CDTHelper[] obj) {
		...
		for (int i=0; i<obj.length; i++) {
			multiObjs[i] = (Object[])obj[i].toObject();
		}
